...
\title{Accelerating IAPWS-IF97 Formula Evaluation via Loop Tiling and Recurrence Polynomial-Derivative Computation}
...
\begin{equation}
\frac{f(\rho,T)}{RT} = \alpha(\delta,\tau) = n_1 \ln\delta + \sum_{i=1}^{N} n_i \, \delta^{I_i} \, \tau^{J_i}
\label{eq:helmholtz}
\end{equation}
...
All thermodynamic properties are derived from the fundamental equations and their partial derivatives. In Regions 1, 2, and 5, properties are derived from the Gibbs free energy $\gamma(\pi,\tau)$, while in Region 3, properties are derived from the Helmholtz free energy $\alpha(\delta,\tau)$. Although the specific expressions differ by region, all share the same computational pattern: evaluating polynomial summations and their partial derivatives. Taking Region 2 as a representative example, the key properties derived from $\gamma(\pi,\tau) = \gamma^0(\pi,\tau) + \gamma^r(\pi,\tau)$ are:
...
\left( \frac{\partial \gamma^r}{\partial \tau} \right)_\pi &= \sum_{i=1}^{N_r} (n_i J_i) \, \pi^{I_i} \, \tau^{J_i - 1}
\end{align}

This means that computing multiple thermodynamic properties requires evaluating several polynomials with the same base terms $\pi^{I_i} \tau^{J_i-1}$ but different coefficient multipliers.
...
\midrule
pT2h\_reg1 & Region 1 & 3.0 MPa, 300 K & Specific Enthalpy & 10x \\
pT2s\_reg1 & Region 1 & 3.0 MPa, 300 K & Specific Entropy & 13x \\
pT2h\_reg2 & Region 2 & 0.0035 MPa, 300 K & Specific Enthalpy & 20x \\
pT2s\_reg2 & Region 2 & 0.0035 MPa, 300 K & Specific Entropy & 15x \\
Tv2h\_reg3 & Region 3 & 650 K, 0.002 m$^3$/kg & Specific Enthalpy & 6x \\
Tv2s\_reg3 & Region 3 & 650 K, 0.002 m$^3$/kg & Specific Entropy & 7x \\
pT2h\_reg5 & Region 5 & 0.5 MPa, 1500 K & Specific Enthalpy & 5x \\
pT2s\_reg5 & Region 5 & 0.5 MPa, 1500 K & Specific Entropy & 6x \\
\bottomrule
\end{tabular}
\end{table}
...
Based on the acceleration techniques presented in Section 3, SEUIF97 is implemented in Rust as a high-performance IAPWS-IF97 property calculation library. The high-performance SEUIF97 library follows a modular three-layer architecture:
