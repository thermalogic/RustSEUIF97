...
but direct formula evaluation cannot meet the performance demands of compute-intensive applications requiring millions of property calculations. Existing approaches to improve computational speed fall into two categories: 
 (1) direct formula evaluation with fast integer power functions (repeated squaring), which provides limited acceleration; and
  (2) alternative methods such as TTSE and SBTL that avoid direct formula evaluation but sacrifice accuracy and support only limited input parameter pairs. This paper presents two acceleration techniques that directly optimize the IAPWS-IF97 formula evaluation:
   ...
   eliminating redundant power calculations. A Rust-based software package, SEUIF97, implements these methods. Benchmark results demonstrate 5x to 20x speedups over baseline implementations, with the largest improvements in Regions 1, 2, and 3.
\end{abstract}
...
The proposed techniques are implemented in SEUIF97(Section \ref{sec:software-implementation-seuif97}). All measurements were conducted on an Intel Core i7-13700K processor with 32 GB DDR5 RAM, using Rust 1.75.0 compiled 
in release mode with \texttt{opt-level = 3} and \texttt{target-cpu = native}. 
Benchmarks were performed using the Criterion framework \cite{Criterion},baselined with integer power function \texttt{powi()} and single loop. Table \ref{tab:benchmarks} shows the results for representative points in each region.
