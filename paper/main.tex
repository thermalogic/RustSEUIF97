...
Taking specific entropy $s$ in Region 2 as an example (Eq.~\ref{eq:entropy}), the computation requires both $\gamma$ and $\gamma_\tau$. As shown in Eq.~\eqref{eq:gibbs_residual} and Eq.~\eqref{eq:gibbs_residual_tau}, $\gamma^r$ and $\gamma^r_\tau$ share the identical base power product $\pi^{I_i}\tau^{J_i}$ and differ only in their coefficients. The shared-power scaling technique computes this base power product once per term, then accumulates contributions to both $\gamma^r$ and $\gamma^r_\tau$ simultaneously in a single loop. The $\gamma^r$ accumulator collects $n_i \pi^{I_i}\tau^{J_i}$ directly, while the $\gamma^r_\tau$ accumulator collects $(n_i J_i) \pi^{I_i}\tau^{J_i}$ and is divided by $\tau$ after the loop to obtain the final derivative value. This single-pass evaluation eliminates redundant computation of the expensive base power term.
