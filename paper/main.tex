...
\begin{itemize}
    ...
    \item \textbf{Shared-Power Scaling + Loop tiling}: Applied to properties requiring multiple related polynomials, such as $s = R[\tau\gamma_\tau - \gamma]$ and $w$ (which requires $\gamma_\pi$, $\gamma_{\pi\pi}$, $\gamma_{\pi\tau}$, $\gamma_{\tau\tau}$). The shared-power scaling technique computes all required derivatives simultaneously, and loop tiling is applied to the combined loop.
\end{itemize}

This selective application ensures that the overhead of shared-power scaling is only incurred when it provides a net benefit, while loop tiling is universally applied to all polynomial summations.
