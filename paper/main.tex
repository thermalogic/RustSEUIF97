...
but direct formula evaluation cannot meet the performance demands of compute-intensive applications requiring millions of property calculations. Existing approaches to improve computational speed fall into two categories: 
 (1) direct formula evaluation with fast integer power functions (repeated squaring), which provides limited acceleration; and
  (2) alternative methods such as TTSE and SBTL that avoid direct formula evaluation but sacrifice accuracy and support only limited input parameter pairs. This paper presents two acceleration techniques that directly optimize the IAPWS-IF97 formula evaluation:
   ...
   eliminating redundant power calculations. A Rust-based software package, SEUIF97, implements these methods. Benchmark results demonstrate 5x to 36x speedups over baseline implementations, with the largest improvements in Regions 1, 2, and 3.
\end{abstract}
...
Despite these efforts, significant opportunities remain unexplored: loop tiling has not been applied to IF97 polynomial evaluation, and the scaling relationship between derivative-related power terms has not been exploited to eliminate redundant calculations. No existing implementation systematically combines these optimizations while maintaining full formula accuracy and support for frequently used input parameter pairs.
...
\textbf{Characteristic 1: Integer exponents.} All fundamental and backward equations are sums of terms $n_i x^{I_i} y^{J_i}$ with integer $I_i$, $J_i$, enabling efficient integer power operations. In C/C++ implementations, this typically requires manual optimization such as repeated squaring, while languages like Rust provide built-in integer power function \texttt{powi()} that can be used directly.
...
Loop tiling (also known as loop blocking) \cite{LoopTiling,Grosser2012Polly} is a general optimization technique that partitions a single loop into multiple smaller loops (tiles) to improve cache locality and enable effective SIMD vectorization. While the technique is well-established, its application to a specific problem requires determining the optimal tile boundaries, which depend on the computational structure, data access patterns, and hardware characteristics.

For IF97 polynomial evaluation, the optimal tile boundaries are determined through empirical performance profiling. The boundaries depend on complex interactions between polynomial term count, CPU cache hierarchy, and compiler optimization behavior. For Region 2 (43 residual terms), profiling reveals that three tiles (0--13, 13--26, and 26--43) yield the best results, enabling the compiler to optimize each tile independently. This experimental methodology is applied to all polynomials across all regions.
...
The proposed techniques are implemented in SEUIF97(Section \ref{sec:software-implementation-seuif97}). All measurements were conducted on an Intel Core i7-1165G7@2.80GHz processor with 8 GB DDR4 RAM. 

\subsection{Benchmarks of IF97 Basic Equation}

using Rust 1.96.0 compiled in release mode with \texttt{opt-level = 3} and \texttt{target-cpu = native} on Windows 11 25H2. 
Benchmarks of IF97 Basic Equations were performed using the Criterion framework \cite{Criterion},baselined with integer power function \texttt{powi()} and single loop. Table \ref{tab:benchmarks} shows the results for representative points in each region.
...
\subsection{Benchmarks of SEUIF97 VS CoolProp/IF97}

The shared library was compiled using Rust 1.96.0 in release mode with \texttt{opt-level = 3} and \texttt{target-cpu = native} 
and Benchmarks C++ code was compiled using GCC 11.4.0 on Ubuntu 22.04.2 LTS.
Benchmarks of SEUIF97 VS CoolProp/IF97 were performed using \texttt{clock\_t} \cite{MusserClock2024}. 

\begin{table}[H]
\centering
\caption{Benchmark SEUIF97 VS CoolProp/IF97}
...
\midrule
$(p,T)\!\rightarrow\!h$ & 1 & 3.0 MPa, 300 K & 868.1 &61.4  &14.1x \\
$(p,T)\!\rightarrow\!s$ & 1 & 3.0 MPa, 300 K & 1461.3&60.4 & 24.1x \\
$(p,T)\!\rightarrow\!h$ & 2 & 0.0035 MPa, 300 K&1270.9 & 70.4 &18.1x \\
$(p,T)\!\rightarrow\!s$ & 2 & 0.0035 MPa, 300 K &2231.4&84.8 & 27.5x \\
$(p,T)\!\rightarrow\!h$ & 3 & 50 MPa, 630 K &3094.5 & 103.3 &29.9x \\
$(p,T)\!\rightarrow\!s$ & 3 & 50 MPa, 630 K &3053.1&101.1 & 30.2x \\
$(p,T)\!\rightarrow\!h$ & 5 & 0.5 MPa, 1500 K & 229.2 & 12.9 & 17.8x \\
$(p,T)\!\rightarrow\!s$ & 5 & 0.5 MPa, 1500 K & 415.8 & 20.8 & 20.0x \\
\bottomrule
\end{tabular}%
}
\end{table}
...
SEUIF97 is available as a Rust crate on crates.io, a Python package on PyPI, a WebAssembly module on npm, and with C bindings. The source code is at \texttt{https://github.com/thermalogic/RustSEUIF97} under the MIT license.
...
enabling simultaneous computation in a single pass. Combined with loop tiling for cache optimization, 
the proposed approach achieves 5x to 36x speedups over baseline implementations that use fast integer power functions with a single loop (Table~\ref{tab:benchmarks}), with the largest improvements in Regions 1, 2, and 3 where the polynomial forms are most complex. 
These results demonstrate that directly accelerating the IAPWS-IF97 formulas can achieve competitive performance without 
resorting to approximation-based methods such as TTSE and SBTL. SEUIF97 is available as a Rust crate, Python package, WebAssembly module, and C library.

Shared-power scaling is applicable to any problem where a function and its derivatives share the same base power terms.
