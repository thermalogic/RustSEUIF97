...
but direct formula evaluation cannot meet the performance demands of compute-intensive applications requiring millions of property calculations. Existing approaches to improve computational speed fall into two categories: 
 (1) direct formula evaluation with fast integer power functions (repeated squaring), which provides limited acceleration; and
  (2) alternative methods such as TTSE and SBTL that avoid direct formula evaluation but sacrifice accuracy and support only limited input parameter pairs. This paper presents two acceleration techniques that directly optimize the IAPWS-IF97 formula evaluation:
   loop tiling and shared-power scaling.
   Loop tiling partitions polynomial summation into cache-friendly tiles that enable effective SIMD (Single Instruction Multiple Data) vectorization.
...
Since its release, it has become the de facto standard in power engineering, process simulation, and thermal system analysis \cite{Wagner2008}. Compute-intensive applications such as real-time simulation, system optimization, and digital twins require millions of property calculations per second.
...
which can become a performance bottleneck in these applications.
Since IF97 equations are based on polynomials with integer exponents, achieving fast property calculations
requires designing efficient evaluation methods for these polynomials.
...
(2) the scaling relationship between power terms of derivative-related polynomials has not been exploited to eliminate redundant calculations;
...
\begin{table}[H]
\centering
\caption{Scaling parameters for fundamental equations in Regions 1, 2, and 5}
...
Table \ref{tab:formula_char} summarizes the fundamental equations across all IAPWS-IF97 regions, including the number of terms, exponent ranges, and formulation types. These characteristics directly influence the computational cost and motivate the acceleration techniques presented in this paper.
...
Comparing Eq.~(\ref{eq:gibbs_residual_tau}) with Eq.~(\ref{eq:gibbs_residual}), both share the identical base power product $\pi^{I_i} \tau^{J_i}$ and differ only in their coefficient multipliers ($n_i$ vs. $n_i J_i$). This means that computing a thermodynamic property requires evaluating polynomials with the same power terms but different coefficient multipliers.

For properties requiring multiple derivative evaluations, such as specific entropy $s$ in Region 2 (Eq.~\ref{eq:entropy}), the computation involves both $\gamma^r$ and $\gamma^r_\tau$, which share the same power products $\pi^{I_i} \tau^{J_i}$. This mathematical relationship enables the shared-power scaling technique described in Section 3.2.

\subsection{Backward Equations}

The backward equations of IAPWS-IF97 and supplementary releases are integer-power polynomials used for inverse property calculations.
For example, the backward equation $T(p,h)$ for subregion 2a in its dimensionless form reads:

\begin{equation}\label{eq:backward_eq_reg2_ph_t_2a}
\frac{T_{2a}(p,h)}{T^*} = \theta_{2a}(\pi,\eta) = \sum_{i=1}^{34} n_i \, \pi^{I_i} (\eta - 2.1)^{J_i}
\end{equation}

where $\theta = T/T^*$, $\pi = p/p^*$, and $\eta = h/h^*$ with $T^* = 1$ K, $p^* = 1$ MPa, and $h^* = 2000$ kJ kg$^{-1}$. The 34 coefficients $n_i$ and exponents $I_i$, $J_i$ are provided in tabular form in the IAPWS supplementary release. Similar backward equations exist for other input parameter pairs across all regions, all sharing the same integer-power polynomial structure. Since these backward equations are also integer-power polynomials, they benefit from the same acceleration techniques as the fundamental equations. 

\subsection{Formula Characteristics}

The IAPWS-IF97 formulation exhibits three key characteristics that motivate the acceleration techniques:

\textbf{Characteristic 1: Integer exponents.} All fundamental and backward equations are sums of terms $n_i x^{I_i} y^{J_i}$ with integer $I_i$, $J_i$, enabling efficient integer power operations. In C/C++ implementations, this typically requires manual optimization such as repeated squaring, while languages like Rust provide built-in integer power functions (e.g., \texttt{powi()}) that can be used directly.

\textbf{Characteristic 2: Shared power terms.} The partial derivatives share the same base power products with the original equations, differing only in coefficients. For example, when computing specific entropy $s$ in Region 2, both $\gamma^r$ and $\gamma^r_\tau$ share the identical base power product $\pi^{I_i}\tau^{J_i}$ (Section 2.2). This enables shared-power scaling.

\textbf{Characteristic 3: High term counts.} The fundamental equations and backward equations in the main regions contain numerous terms, making polynomial evaluation the dominant computational cost and benefiting from loop tiling. For example, Regions 1 and 2 contain 34 and 52 terms respectively (Table \ref{tab:formula_char}).
...
Taking specific enthalpy $h$ in Region 2 as an example (Eq.~\ref{eq:enthalpy}), the computation requires only the first-order partial derivative $\gamma_\tau$. The loop tiling technique partitions the term indices into tiles that fit within CPU cache, reducing memory access latency and allowing the compiler to optimize each tile independently.
...
The optimal tile boundaries are determined through empirical performance profiling rather than theoretical analysis, as they depend on complex interactions between polynomial term count, CPU cache hierarchy, and compiler optimization behavior. For Region 2 (43 residual terms), profiling reveals that three tiles (0--13, 13--26, and 26--43) yield the best results, enabling the Rust compiler to optimize each tile independently through loop unrolling, SIMD vectorization, and register allocation. This experimental methodology is applied to all polynomials across all regions.
...
When a thermodynamic property requires both the polynomial value and its derivatives, shared-power scaling eliminates redundant computation. The base power product is computed once per term, and the derivative contributions are obtained by scaling with the corresponding exponents. This single-pass evaluation avoids repeated power calculations.

Taking specific entropy $s$ in Region 2 as an example (Eq.~\ref{eq:entropy}), the computation requires both $\gamma$ and $\gamma_\tau$. As shown in Eq.~\eqref{eq:gibbs_residual} and Eq.~\eqref{eq:gibbs_residual_tau}, $\gamma^r$ and $\gamma^r_\tau$ share the identical base power product $\pi^{I_i}\tau^{J_i}$ and differ only in their coefficients. The shared-power scaling technique computes each term (coefficient $\times$ base power product) once per iteration, then accumulates contributions to both $\gamma^r$ and $\gamma^r_\tau$ simultaneously in a single loop. The $\gamma^r$ accumulator collects $n_i \pi^{I_i}\tau^{J_i}$ directly, while the $\gamma^r_\tau$ accumulator collects $(n_i J_i) \pi^{I_i}\tau^{J_i}$ and is divided by $\tau$ after the loop to obtain the final derivative value. This single-pass evaluation eliminates redundant computation of the expensive power terms.
...
\begin{itemize}
    \item \textbf{Loop Tiling}: Applied to polynomial summations where profiling demonstrates performance improvement (typically Regions 1, 2, and 3 with many terms). For properties with few terms (Region 5), loop tiling is omitted as the tile overhead may outweigh the benefits.
    \item \textbf{Shared-Power Scaling}: Applied to all properties requiring both the polynomial and its derivatives, such as $s = R[\tau\gamma_\tau - \gamma]$ and $w$ (which requires $\gamma_\pi$, $\gamma_{\pi\pi}$, $\gamma_{\pi\tau}$, $\gamma_{\tau\tau}$). The shared-power scaling technique computes all required derivatives simultaneously, eliminating redundant power calculations.
\end{itemize}

These techniques are orthogonal and can be applied independently. For example, Region 5 entropy computation uses shared-power scaling alone (Table~\ref{tab:benchmarks}), while Region 2 enthalpy uses loop tiling alone. When both techniques are applicable, they are combined for maximum performance improvement.

\section{Performance Benchmarks}

The proposed techniques are implemented in SEUIF97, a Rust-based IAPWS-IF97 property calculation library. All measurements were conducted on an Intel Core i7-13700K processor (24 cores, up to 5.4 GHz) with 32 GB DDR5 RAM, running Windows 11, using Rust 1.75.0 compiled in release mode with \texttt{opt-level = 3} and \texttt{target-cpu = native}. Benchmarks were performed using the Criterion framework \cite{Criterion}. Table \ref{tab:benchmarks} shows the benchmark results for representative points in each region.

For Region 2 enthalpy computation ($h$), loop tiling reduces the execution time from approximately 1.07~µs to 53.8~ns, achieving a ~20x speedup. For Region 2 entropy $s$ computation, which requires both $\gamma$ and $\gamma_\tau$, shared-power scaling combined with loop tiling reduces the execution time from approximately 2.2~µs to 138.7~ns, achieving a ~15x speedup. These improvements stem from better cache utilization, SIMD vectorization, and the elimination of redundant power calculations.

\begin{table}[H]
\centering
\caption{Performance benchmark results (speedup relative to baseline with fast integer power functions and single loop)}
...
$(p,T)\!\rightarrow\!s$ & 1 & 3.0 MPa, 300 K & Loop tiling + Shared-power scaling & 20x \\
$(p,T)\!\rightarrow\!h$ & 2 & 0.0035 MPa, 300 K & Loop tiling & 20x \\
$(p,T)\!\rightarrow\!s$ & 2 & 0.0035 MPa, 300 K & Loop tiling + Shared-power scaling & 15x \\
$(T,v)\!\rightarrow\!h$ & 3 & 650 K, 0.002 m$^3$/kg & Loop tiling & 17x \\
$(T,v)\!\rightarrow\!s$ & 3 & 650 K, 0.002 m$^3$/kg & Loop tiling + Shared-power scaling & 13x \\
$(p,T)\!\rightarrow\!h$ & 5 & 0.5 MPa, 1500 K & Baseline (\texttt{powi()}) & 1x \\
$(p,T)\!\rightarrow\!s$ & 5 & 0.5 MPa, 1500 K & Shared-power scaling & 5x \\
\bottomrule
\end{tabular}%
}
\end{table}
...
To meet the needs of comprehensive water and steam property calculations across diverse engineering applications, SEUIF97 supports 12 input parameter pairs: $(p,t)$, $(p,h)$, $(p,s)$, $(p,v)$, $(t,h)$, $(t,s)$, $(t,v)$, $(h,s)$, $(p,x)$, $(t,x)$, $(h,x)$, $(s,x)$ (where $x$ is vapor quality), and computes 36 thermodynamic, transport, and derived properties, including basic properties, derived properties, partial derivatives, and transport properties.
...
This paper proposes two acceleration techniques for the IAPWS-IF97 formula evaluation: loop tiling and
shared-power scaling in derivative-related polynomials. The key insight is that the IAPWS-IF97 fundamental equations and their partial derivatives share the same power terms, enabling simultaneous computation in a single pass. Combined with loop tiling for cache optimization, the proposed approach achieves 5x to 20x speedups over baseline implementations that use fast integer power functions with a single loop (Table~\ref{tab:benchmarks}), with the largest improvements in Regions 1, 2, and 3 where the polynomial forms are most complex. These results demonstrate that directly accelerating the IAPWS-IF97 formulas can achieve competitive performance without resorting to approximation-based methods such as TTSE and SBTL. The Rust-based software package SEUIF97, available in multiple package formats, provides convenience for developers working in different programming environments.

The shared-power scaling technique is not limited to IAPWS-IF97 property calculations. It is applicable to any computational problem where a function and its derivatives share the same base power terms, which is common in thermodynamic formulations, equation of state evaluations, and other scientific computing domains involving polynomial-based derivative computations.
