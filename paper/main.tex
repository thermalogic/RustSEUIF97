...
When a thermodynamic property requires both the polynomial value and its derivatives, shared-power scaling computes the base power product once per term and obtains derivative contributions by scaling with the corresponding exponents, eliminating redundant power calculations.

Taking specific entropy $s$ in Region 2 as an example (Eq.~\ref{eq:entropy}), the computation requires both $\gamma$ and $\gamma_\tau$. Since $\gamma^r$ and $\gamma^r_\tau$ share the identical base power product $\pi^{I_i}\tau^{J_i}$ (Eqs.~\eqref{eq:gibbs_residual} and \eqref{eq:gibbs_residual_tau}), shared-power scaling computes each term once and accumulates contributions to both simultaneously. The $\gamma^r_\tau$ accumulator collects $(n_i J_i) \pi^{I_i}\tau^{J_i}$ and is divided by $\tau$ after the loop.
...
For Region 2 enthalpy computation ($h$), loop tiling achieves a ~20x speedup. For Region 2 entropy $s$ computation, shared-power scaling combined with loop tiling achieves a ~15x speedup. These improvements stem from better cache utilization, SIMD vectorization, and elimination of redundant power calculations.
