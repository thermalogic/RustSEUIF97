...
maintaining full formula accuracy:
   ...
   Benchmark results demonstrate 5x to 36x speedups over baseline implementations and 14x to 30x speedups over CoolProp IF97, the most widely used open-source implementation, with the largest improvements in Regions 1, 2, and 3.
\end{abstract}
...
These techniques are orthogonal and can be applied independently. For example, Region 5 entropy computation uses shared-power scaling alone, while Region 2 enthalpy uses profiling-guided loop tiling alone (Table~\ref{tab:benchmarks}). When both techniques are applicable, they are combined for maximum performance improvement.

\section{Performance Benchmarks}
\label{sec:performance-benchmarks}

All benchmarks were conducted on the same hardware: an Intel Core i7-1165G7 @ 2.80~GHz processor with 8~GB DDR4 RAM. Note that the two benchmarks use different software platforms and measurement frameworks; therefore, absolute execution times are not directly comparable across tables.

\subsection{Speedup over Baseline Implementation}

The first benchmark evaluates the proposed acceleration techniques by comparing the optimized SEUIF97 implementation against a baseline that uses only Rust's built-in integer power function \texttt{powi()} with a single loop, without loop tiling or shared-power scaling. Both implementations evaluate the IF97 basic equations. This benchmark was performed on Windows~11, with SEUIF97 compiled using Rust 1.96.0 in release mode with \texttt{opt-level = 3} and \texttt{target-cpu = native}. Measurements were conducted using the Criterion framework \cite{Criterion}.
...
This benchmark was performed on Ubuntu~22.04. SEUIF97 was compiled using Rust 1.96.0 in release mode with \texttt{opt-level = 3} and \texttt{target-cpu = native}, and CoolProp IF97 was compiled using GCC 11.4.0 with \texttt{-O3} optimization. Measurements were performed using \texttt{clock\_t} \cite{MusserClock2024}.
Table~\ref{tab:benchmarks_seuif97_coolprop_if97} shows the comparison results. Note that Region 3 results use backward equations $(p,T)$ since CoolProp IF97 does not implement the $(T,v)$ fundamental equation interface.
...
SEUIF97 achieves 14--30x speedups over CoolProp IF97. These gains stem from two sources: the proposed algorithmic optimizations, and compiler-level optimization of Rust's \texttt{powi()} intrinsic compared to CoolProp's hand-coded repeated-squaring routine.

Region 5 isolates the compiler contribution. Enthalpy $(p,T)\rightarrow h$ uses only 12 terms with a single derivative, so neither loop tiling nor shared-power scaling applies; the 17.8x speedup reflects primarily LLVM's aggressive optimization of \texttt{powi()}. Entropy $(p,T)\rightarrow s$ in Region 5 reaches 20.0x, with the additional gain attributable to shared-power scaling.

In Regions 1--3, all optimizations combine. Entropy calculations reach 24--30x because shared-power scaling eliminates redundant power calculations while loop tiling improves cache locality for the large polynomial sums. Enthalpy cases (14--23x) are lower because only loop tiling is active.
