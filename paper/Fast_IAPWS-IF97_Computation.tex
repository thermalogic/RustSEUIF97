...
\title{Fast IAPWS-IF97 Computation via Loop Tiling and Recurrence Polynomial-Derivative Computation}
...
\begin{abstract}
The IAPWS-IF97 formulation is the international standard for water and steam thermodynamic properties, but direct formula evaluation cannot meet the compute-intensive demands of large-scale simulations requiring millions of property calculations. Existing approaches to improve computational speed fall into two categories: (1) direct formula evaluation with fast integer power functions (repeated squaring), which provides limited acceleration; and (2) alternative methods such as TTSE and SBTL that avoid direct formula evaluation but sacrifice accuracy and support only limited input parameter pairs. This paper presents two acceleration techniques that directly optimize the IAPWS-IF97 formula evaluation: loop tiling and recurrence polynomial-derivative computation. Loop tiling partitions polynomial summation into cache-friendly tiles that enable effective SIMD vectorization. Recurrence polynomial-derivative computation exploits the mathematical relationship between Gibbs/Helmholtz free energy polynomials and their partial derivatives to compute them simultaneously in a single pass, eliminating redundant calculations. These techniques maintain full formula accuracy while supporting all 12 input parameter pairs. A Rust-based software package, SEUIF97, implements these methods and computes 36 thermodynamic, transport, and derived properties. Benchmark results demonstrate 5x to 20x speedups compared to baseline implementations using fast integer power functions with a single for loop, with the largest improvements in Regions 1, 2, and 3 where polynomial forms are most complex.
\end{abstract}
...
\bibitem{Criterion}
J. Boucher, ``Criterion.rs: Statistics-driven micro-benchmarking in Rust,'' GitHub, 2022. [Online]. Available: \url{https://github.com/bheisler/criterion.rs}. Accessed: Jun. 6, 2026.
