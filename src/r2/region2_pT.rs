...
pub fn pT2v_reg2(p: f64, T: f64) -> f64 {
    ...
    let pi: f64 = p;
    0.001 * RGAS_WATER * T * pi * (gamma0_pi_reg2(pi) + gammar_pi_reg2(pi,tau)) / p
}

pub fn pT2h_reg2(p: f64, T: f64) -> f64 {
    let tau: f64 = r2Tstar / T;
    let pi: f64 = p;
    RGAS_WATER * T * tau * (gamma0_tau_reg2(tau) + gammar_tau_reg2(pi,tau))
}
...
pub fn pT2s_reg2(p: f64, T: f64) -> f64 {
    ...
    gamma0_tau /= tau;
    let (gammar, gammar_tau) = sum_0_j_power(pi, tau - 0.5, &IJn);
...
pub fn pT2u_reg2(p: f64, T: f64) -> f64 {
    let tau: f64 = r2Tstar / T;
    let pi: f64 = p;
    let (gammar_pi, gammar_tau) = sum_i_j_power(pi, tau - 0.5, &IJn);
    RGAS_WATER
        * T
...
pub fn pT2cv_reg2(p: f64, T: f64) -> f64 {
    ...
    let (gammar_pi, gammar_pipi, gammar_pitau, gammar_tautau) =
        sum_i_ii_ij_jj_power(pi, tau - 0.5, &IJn);
